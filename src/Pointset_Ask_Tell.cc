...
::Pointset_Ask_Tell(const Pointset_Ask_Tell<C_Polyhedron>& y)
  ...
  for (Pointset_Ask_Tell<C_Polyhedron>::const_iterator i = y.begin(),
    ...
    COW_Pointset<NNC_Polyhedron> nnc_ask(NNC_Polyhedron(i->ask().pointset()));
    COW_Pointset<NNC_Polyhedron> nnc_tell(NNC_Polyhedron(i->tell().pointset()));
...
::Pointset_Ask_Tell(const Pointset_Ask_Tell<NNC_Polyhedron>& y)
  ...
  for (Pointset_Ask_Tell<NNC_Polyhedron>::const_iterator i = y.begin(),
	 y_end = y.end(); i != y_end; ++i) {
    COW_Pointset<C_Polyhedron> c_ask(C_Polyhedron(i->ask().pointset()));
    COW_Pointset<C_Polyhedron> c_tell(C_Polyhedron(i->tell().pointset()));
