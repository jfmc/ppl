...
ERational::ERational(Coefficient_traits::const_reference num,
  ...
  : e(0) {
  assert(den != 0);
  Checked::assign<Checked::Transparent_Policy>(v.get_num(), raw_value(num), Rounding(Rounding::IGNORE));
  Checked::assign<Checked::Transparent_Policy>(v.get_den(), raw_value(den), Rounding(Rounding::IGNORE));
  v.canonicalize();
}
