...
/* FIX Name these methods convert and name this file Grid_convert.cc
   to be consistently use verbs as function names, and make the same
   changes to the Polyhedron equivalents. */
...
/*!
  ...
  If some of the congruences in \p source are redundant, they will be removed.
  ...
  a generator system corresponding to an empty congruence system.
  ...
  the non-pending congruences of \p source.
  ...
  congruences or a system of generators (points, rays and lines)
  ...
  whose rows represent the congruences that characterize \f$P\f$
  ...
  and we call \f$C\f$ and \f$G\f$ the systems of congruences and
  ...
  that the conversion being performed is from congruences to generators.
  Thus, even if the roles of \p source and \p dest can be interchanged,
  in the sequel we assume the \p source system will contain the congruences
  ...
  Observation 1: Let \f$A\f$ be a system of congruences that generate
  the polyhedron \f$P\f$ and \f$\vect{c}\f$ a new congruence that must
  be added. Suppose that there is a line \f$\vect{z}\f$ that does not
  saturate the congruence \f$\vect{c}\f$. If we combine the old lines
  and rays that do not saturate \f$\vect{c}\f$ (except \f$\vect{z}\f$)
  with \f$\vect{z}\f$ such that the new ones saturate \f$\vect{c}\f$,
  the new lines and rays also saturate the congruences  saturated by
  ...
  \f]
  and \f$\vect{c}_1\f$ is a previous congruence that \f$\vect{y}_1\f$
  ...
  b) \f$\vect{r}_1\f$ and \f$\vect{r}_2\f$ are extreme rays and the
     rank of the system composed by the congruences saturated by both
     \f$\vect{r}_1\f$ and \f$\vect{r}_2\f$ is equal to
     \f$d - 2\f$, where \f$d\f$ is the rank of the system of congruences.

  In fact, let \f$F\f$ be the system of generators that saturate the
  congruences saturated by both \f$\vect{r}_1\f$ and \f$\vect{r}_2\f$.
  ...
  congruence \f$c\f$ saturated by \f$\vect{r}_1\f$ and not
  ...
  the congruences saturated by both \f$\vect{r}_1\f$ and \f$\vect{r}_2\f$
  ...
  a system \f$A\f$ of congruences of \f$P\f$, if \f$\vect{c}\f$ is the
  congruence to add to \f$A\f$ and all lines of \f$P\f$ saturate
  ...
  -# lies on the hyper-plane represented by the k-th congruence,
  -# is a positive combination of two adjacent rays \f$\vect{r}_1\f$ and
     \f$\vect{r}_2\f$ such that the first one satisfies the congruence and
  ...
  the rank of the system composed by the congruences saturated by both
  ...
  it saturates the same congruences saturated by both \f$\vect{r}_1\f$ and
  \f$\vect{r}_2\f$.
  If the rank is less than \f$d - 2\f$, the rank of
  the system composed by \f$\vect{c}\f$ (that is saturated by \f$\vect{r}\f$)
  and by the congruences of \f$A\f$ saturated by \f$\vect{r}\f$  is less
...
*/
PPL::dimension_type
PPL::Grid::conversion(Linear_System& source,
  ...
  // If `start > 0', then we are converting the pending congruences.
  assert(start == 0 || start == source.first_pending_row());

  // During the iteration on the congruences in `source' we may identify
  // congruences that are redundant: these have to be removed by swapping
  // the rows of `source', taking care not to compromise the sortedness
  // of the congruences that still have to be considered.
  // To this end, the following counter keeps the number of redundant
  // congruences seen so far, to be used as a displacement when swapping rows.
  ...
  for (dimension_type k = start; k < source_num_rows; ) {
    ...
    // All the `source_num_redundant' redundant congruences identified so far
    // have consecutive indices starting from `k'.
    if (source_num_redundant > 0)
      // Let the next congruence have index `k'.
    ...
    // Congruences and generators must have the same dimension,
    ...
    // of the congruence `source_k' and the generator `dest[i]'.
    // This product is 0 iff the generator saturates the congruence.
    ...
    // that does not saturate the congruence `source_k'.
	...
	// The generator does not saturate the congruence.
	break;
    ...
    if (index_non_zero < num_lines_or_equalities) {
      ...
      // the congruence (namely, the ray whose scalar product
      // with the congruence gives a positive result).
      if (scalar_prod[index_non_zero] < 0) {
	...
	std::swap(scalar_prod[index_non_zero],
      ...
      // the congruence `source_k', the scalar product between
      // the line and the congruence must be 0.
      ...
      // Each line that does not saturate the congruence has to be
      // linearly combined with generator `dest[num_lines_or_equalities]'
      // so that the resulting new line saturates the congruence.
      // Note that, by Observation 1 above, the resulting new line
      // will still saturate all the congruences that were saturated by
      // the old line.
#if 0
	...
	if (scalar_prod[i] != 0) {
      ...
      // that do not saturate the congruence `source_k'. These rays are
      // positively combined with the ray `dest[num_lines_or_equalities]'
      // so that the resulting new rays saturate the congruence.
      for (dimension_type
	...
	maybe_abandon();
      ...
      // (by construction), it does not saturate the congruence `source_k'.
      // Therefore, if the congruence is an inequality,
      // we set to 1 the corresponding element of `sat' ...
      if (source_k.is_ray_or_point_or_inequality())
	sat[num_lines_or_equalities].set(k);
      // ... otherwise, the congruence is an equality which is
	...
	// `dest' has already been set as non-sorted.
      ...
      // We continue with the next congruence.
      ++k;
    }
    // Here we have `index_non_zero' >= `num_lines_or_equalities',
    // so that all the lines in `dest' saturate the congruence `source_k'.
    else {
      ...
      // -# all the rays that saturate the congruence should have
      ...
      //    congruence should have indexes between `lines_or_equal_bound'
      //    and `sup_bound' - 1; these rays form the set Q+.
      // -# all the rays that have a negative scalar product with the
      //    congruence should have indexes between `sup_bound' and
	...
	// If the congruence is an inequality, then all the generators
	// in Q= and Q+ satisfy the congruence. The congruence is redundant
	// and it can be safely removed from the congruence system.
	...
	if (source_k.is_ray_or_point_or_inequality()) {
	  ...
	  //    in position `k' a congruence that still has to be examined.
	}
	else {
	  // The congruence is an equality, so that all the generators
	...
	// violates the congruence `source_k'.
	...
	if (sup_bound == num_lines_or_equalities)
	  ...
	  // the congruence also saturate it.
	  ...
	  // the congruence `source_k' plus a set of new rays enjoying
	  // the following properties:
	  // -# they lie on the hyper-plane represented by the congruence
	  ...
	  for (dimension_type i = lines_or_equal_bound; i < sup_bound; ++i) {
	    for(dimension_type j = sup_bound; j < bound; ++j) {
	      ...
	      // all the congruences saturated by both `dest[i]' and
		...
		num_common_satur = k - new_satrow.count_ones();
	      ...
	      // (see comments in Grid.defs.hh):
	      // an extremal ray saturates at least `n' - `t' - 1
	      // congruences, where `n' is the dimension of the space
		...
		if (!redundant) {
		  ...
		  // represented by the congruence `source_k'.
	  ...
	  // the congruence) with the newly added rays.
	  dimension_type j;
	  if (source_k.is_ray_or_point_or_inequality()) {
	    // The congruence is an inequality:
	    // the violating generators are those in Q-.
	    j = sup_bound;
	    // For all the generators in Q+, set to 1 the corresponding
	    // entry for the congruence `source_k' in the saturation matrix.
	  ...
	  // with the generators violating the congruence
	  ...
	  // - if the number of generators violating the congruence
	  ...
	  //   all generators below index `j-1' violates the congruence.
  ...
  // We may have identified some redundant congruences in `source',
  ...
  // There are no longer pending congruences in `source'.
