...
PPL::Polyhedron::Polyhedron(ConSys& cs)
  ...
  if (cs.num_rows() == 0)
    ...
    // A non-empty matrix of constraints must have at least two columns:
    // one for the inhomogeneus term and the other for a variable.
    assert(cs.num_columns() >= 2);
    // Setting the dimension of the space.
    space_dim = cs.num_columns() - 1;
...
PPL::Polyhedron::Polyhedron(GenSys& gs)
  ...
  if (gs.num_rows() == 0)
    ...
    // A non-empty matrix of generators must have at least two columns:
    // one for the inhomogeneus term and the other for a variable.
...
PPL::Polyhedron::remove_dimensions(const std::set<Variable>&
  ...
  if (gen_sys.num_columns() > 1)
    ...
    // then polyhedron is zero-dimensional.
