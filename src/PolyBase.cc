...
PPL::PolyBase::PolyBase(size_t num_dimensions,
  ...
  : necessarily_closed_(necessarily_closed),
    con_sys(),
    gen_sys(),
    sat_c(),
    sat_g() {
...
PPL::PolyBase::PolyBase(const PolyBase& y)
  : necessarily_closed_(y.necessarily_closed_),
...
*/
PPL::PolyBase::PolyBase(ConSys& cs, bool necessarily_closed)
  : necessarily_closed_(necessarily_closed),
    con_sys(),
    gen_sys(),
    sat_c(),
...
*/
PPL::PolyBase::PolyBase(GenSys& gs, bool necessarily_closed)
  : necessarily_closed_(necessarily_closed),
    con_sys(),
    gen_sys(),
    sat_c(),
...
PPL::PolyBase::convex_difference_assign(const PolyBase& y) {
  ...
  PolyBase new_polyhedron(x_space_dim, EMPTY, necessarily_closed_);
  ...
  for (ConSys::const_iterator i = y_cs.begin(),
	...
	if (n != 0)
      ...
      new_polyhedron.convex_hull_assign(PolyBase(z_cs, necessarily_closed_));
    }
  }
  *this = new_polyhedron;
...
PPL::PolyBase::add_dimensions_and_constraints(ConSys& cs) {
  ...
  if (space_dim == 0) {
    PolyBase y(cs, necessarily_closed_);
    swap(y);
    return;
  }
