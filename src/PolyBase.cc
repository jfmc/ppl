...
PPL::PolyBase::remove_higher_dimensions(size_t new_dimension) {
  ...
  if (is_necessarily_closed())
    ...
    size_t old_eps_col = gen_sys.num_columns() - 1;
    size_t new_eps_col = new_dimension + 1;
    for (size_t r = gen_sys.num_rows(); r-- > 0; )
      std::swap(gen_sys[r][new_eps_col], gen_sys[r][old_eps_col]);
    // The number of remaining columns is `new_eps_col + 1'.
    // Note that resizing also calls `set_sorted(false)'.
    gen_sys.resize_no_copy(gen_sys.num_rows(), new_eps_col + 1);
  }
  // We may have invalid line and rays now.
  gen_sys.remove_invalid_lines_and_rays();
