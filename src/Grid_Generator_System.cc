...
PPL::Grid_Generator_System::insert(const Grid_Generator& g) {
...

  {
	...
	swap_columns(old_num_columns - 1, g_size - 1);
      Matrix<Dense_Row>::add_row(g);
    }
    else if (g_size < old_num_columns)
      if (old_num_rows == 0)
	Matrix<Dense_Row>::add_row(Linear_Row(g, old_num_columns, old_num_columns));
      else {
	...
	Matrix<Dense_Row>::add_row(tmp_row);
      }
    else
...
::remove_space_dimensions(const Variables_Set& vars) {
  ...
  for (++vsi; vsi != vsi_end; ++vsi) {
    ...
    while (src_col < vsi_col)
      Matrix<Dense_Row>::swap_columns(dst_col++, src_col++);
    ++src_col;
  }
  ...
  Matrix<Dense_Row>::remove_trailing_columns(num_columns - dst_col);
}
...
::remove_higher_space_dimensions(const dimension_type new_dimension) {
  ...
  Matrix<Dense_Row>::remove_trailing_columns(space_dim - new_dimension);
  PPL_ASSERT(OK());
}
