...
::remove_space_dimensions(const Variables_Set& vars) {
  ...
  for (++vsi; vsi != vsi_end; ++vsi) {
    ...
    while (src_col < vsi_col)
      Dense_Matrix::swap_columns(dst_col++, src_col++);
    ++src_col;
  }
  ...
  Dense_Matrix::remove_trailing_columns(num_columns - dst_col);
}
...
::remove_higher_space_dimensions(const dimension_type new_dimension) {
  ...
  Dense_Matrix::remove_trailing_columns(space_dim - new_dimension);
  PPL_ASSERT(OK());
}
