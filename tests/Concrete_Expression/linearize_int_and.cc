/* Testing linearization algorithm and its related function of the bitwise and.
...
test06() {
  ...
  linearize_int(band, oracle1, Integer_Linear_Form_Abstract_Store(), result);
  nout << "*** " << known_result << " & " << lf << " ***" << endl
       << "*** result *** " << endl
       << result << endl;
  known_result1 &= lf;
...
test12() {
  ...
  linearize_int(band, oracle, Integer_Linear_Form_Abstract_Store(),
		result);
  nout << "*** " << known_result << " & " << lf << " ***" << endl
	 << "*** result *** " << endl
	 << result << endl;
...
bool
test13() {
  Integer_Linear_Form_Abstract_Store store;
  Variable A(0);
  Integer_Linear_Form known_result = Integer_Linear_Form(A);
  store[0] = known_result;

  Approximable_Reference<C_Expr> var(Integer_Type,
				      Int_Interval(mpz_class(0)), 0);
  Integer_Linear_Form result;
  linearize_int(var, Test_Oracle(), store, result);

  nout << "*** known_result ***" << endl
       << known_result << endl << endl;;
