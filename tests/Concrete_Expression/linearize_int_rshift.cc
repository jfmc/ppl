...
test01() {
  ...
  linearize_int(rshift, oracle2, Integer_Linear_Form_Abstract_Store(),
		result );
  ...
  linearize_int(rshift, oracle3, Integer_Linear_Form_Abstract_Store(),
		result );
...
/*
  Tests linearization A + 3*B + [2, 3] >> -2*A - B + [-9, -1]
  where:
   -) A in [0, 1] and B in [2, 3];
   -) A in [2, 3] and B in [1, 3];
   -) A in [1, 5] and B in [4, 10];
...
test07() {
  ...
  Int_Constant<C_Expr> coeff_inh_term(Integer_Type, Integer_Int_Interval(3));

  Binary_Operator<C_Expr> expr1(Integer_Type, Binary_Operator<C_Expr>::ADD,
				&var0, &coeff_inh_term);

  Binary_Operator<C_Expr> expr2(Integer_Type, Binary_Operator<C_Expr>::MUL,
				&var1, &coeff_b1);
...
/*
  Tests linearization A + 1123456*B + [24, 39] >> A + 8*B + [1, 4]
  where:
  -) A in [0, 1] and B in [2, 3];
*/
bool
test11() {
  ...
  Integer_Int_Interval b(1123456);

  Integer_Int_Interval inh_term(24);
  inh_term.join_assign(39);
...
/*
  Tests linearization -198718*A + -12*B + [-3, -2] >> A + 8*B + [1, 4]
  where:
  -) A in [0, 1] and B in [2, 3];
  -) A in [2, 3] and B in [0, 1].
