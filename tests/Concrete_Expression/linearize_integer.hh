    ...
    case Binary_Operator<Target>::KIND:{
      ...
      switch (bop_expr->binary_operator()) {
        ...
        case Binary_Operator<Target>::LSHIFT:{
          const Int_Constant<Target>* intc_expr =
          bop_expr->right_hand_side()->template as<Int_Constant>();
          int exp;
          if (atoi(intc_expr->value) < 0) 
            exp = -ldexp(1.0,-atoi(intc_expr->value));
          else
            exp = ldexp(1.0,atoi(intc_expr->value));
          Int_Constant<Target> lshift(itoa(exp),strlen(itoa(exp)));
          Binary_Operator<C_Expr> left_shift(bop_expr->type(), bop_expr->kind(), bop_expr->left_hand_side(), &lshift);
          return lshift_linearize_int(left_shift, oracle, lf_store, result);
          break;
        }
        case Binary_Operator<Target>::RSHIFT:{
          ...
          if (atoi(intc_expr->value) < 0)
            ...
            exp = ldexp(1.0,atoi(intc_expr->value));
          Int_Constant<Target> rshift(itoa(exp),strlen(itoa(exp)));
          Binary_Operator<C_Expr> right_shift(bop_expr->type(), bop_expr->kind(), bop_expr->left_hand_side(), &rshift);
          return rshift_linearize_int(right_shift, oracle, lf_store, result);
          break;
        } 
      }
    }
