/* Computing the polynomial image of Polynomial_Spaces.
...
test01() {
  ...
  try {
    ...
    // Polynomial_Space::polynomial_image:
    // it is illegal to compute the affine pre-image with
    // a zero denominator.

  ps1.polynomial_image(x1,expr,0);
  }
...
test02() {
  ...
  try {
    // This is an invalid use of function
    // Polynomial_Space::polynomial_image:
    // it is illegal to compute the polynomial image with
    // a right-hand side which is dimension-incompatible.

  ps1.polynomial_image(x1,expr);
  }
  catch (std::invalid_argument& e) {
    nout << "invalid_argument: " << e.what() << endl;
  }
...
test04() {
  ...
  ps1.polynomial_image(x2,e1,2);

  Polynomial e2( 2*x2 - 1 - x1 - x3);
  ps1.polynomial_image(x2,e2,3);
...
test05() {
  ...
  ps1.polynomial_image(x3,e1,3);

  Polynomial e2( 3*x3 - 1 - x1);
  ps1.polynomial_image(x3,e2,5);
...
test08() {
  ...
  // Testing if applying an invertible polynomial map and then its inverse yields the
  // same space.
...
test09() {
  ...
  // Testing non-invertible polynomial maps.

  Polynomial_Space<3> ps1(3);
  ps1.add_polynomial_constraint
    (Polynomial(x1 + 2) == 0);
  ps1.add_polynomial_constraint
    (Polynomial(2*x3 - x2*x2) == 0);
