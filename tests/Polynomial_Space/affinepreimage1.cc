/* Computing the affine pre-image of Polynomial_Spaces.
...
test01() {
  ...
  try {
    ...
    // Polynomial_Space::affine_preimage:
    // it is illegal to compute the affine pre-image with
    // a zero denominator.

  ps1.affine_preimage(x1,expr,0);
  }
...
test04() {
  ...
  ps1.affine_preimage(x1,e1);

  Linear_Expression e2( x1 - 1 );
  ps1.affine_preimage(x1,e2);
...
test05() {
  ...
  ps1.affine_preimage(x2,e1,2);

  Linear_Expression e2( 2*x2 - 1 - x1 - x3);
  ps1.affine_preimage(x2,e2,3);
...
test06() {
  ...
  ps1.affine_preimage(x3,e1,3);

  Linear_Expression e2( 3*x3 - 1 - x1);
  ps1.affine_preimage(x3,e2,5);
