/* Adding a system of polynomial constraints to a Polynomial_Space.
...
test01() {
  ...
  try {
    ...
    cs.insert(       x1 * x1 - x2 == 0);
    cs.insert(Polynomial(x1 - x3) == 0);
    ps1.add_polynomial_constraints(cs);
...
test02() {
  ...
  Polynomial_Constraint_System cs;
  cs.insert(Polynomial(x1 + 2*x2 + 1) == 0);
  cs.insert(Polynomial(x2 - x1   - 1) == 0);
  ps1.add_polynomial_constraints(cs);

  Polynomial_Space<2> known_result(2);
  known_result.add_polynomial_constraint(Polynomial(x1 + 2*x2 + 1) == 0);
  known_result.add_polynomial_constraint(Polynomial(x2 - x1   - 1) == 0);
