/* Computing the affine image of Polynomial_Spaces.
...
test02() {
  ...
  Linear_Expression expr(x3 + 1);

  try {
    // This is an invalid use of function
    // Polynomial_Space::affine_image:
    // it is illegal to compute the affine image with
    // a right-hand side which is dimension-incompatible.

  ps1.affine_image(x1,expr);
  }
  catch (std::invalid_argument& e) {
    nout << "invalid_argument: " << e.what() << endl;
  }
...
test04() {
  ...
  // Testing if applying an invertible affine map and then its inverse yields the
  // same space.

  Polynomial_Space<2> ps1(3);
  ps1.add_polynomial_constraint(Polynomial(3 + 2*x1 + x2) == 0);

  Linear_Expression e1( x1 + 1 );
  ps1.affine_image(x1,e1);

  Linear_Expression e2( x1 - 1 );
  ps1.affine_image(x1,e2);
...
test05() {
  ...
  Linear_Expression e1( 3*x2 + 1 + x1 + x3);
  ps1.affine_image(x2,e1,2);

  Linear_Expression e2( 2*x2 - 1 - x1 - x3);
  ps1.affine_image(x2,e2,3);
...
test06() {
  ...
  Linear_Expression e1( 5*x3 + 1 + x1);
  ps1.affine_image(x3,e1,3);

  Linear_Expression e2( 3*x3 - 1 - x1);
  ps1.affine_image(x3,e2,5);
...
test07() {
  ...
  // Testing non-invertible affine maps.
...
test08() {
  ...
  ps1.add_polynomial_constraint
    (Polynomial(x1 + 2) == 0);
  ps1.add_polynomial_constraint
    (Polynomial(2*x3 - x2) == 0);

  Linear_Expression e1(x2);
  ps1.affine_image(x1,e1,2);
